...
- A universe note to scope the map to. See [I want to create a fictional universe](/guides/worldbuilding/create-fictional-universe/). Optional, but recommended — it lets places filter automatically by universe rather than coordinate match.
...
![Custom image map for the Dying Earth fantasy fixture, with place markers on a hand-drawn basemap](/img/cr-map-custom-image.webp)
...
- **Drilldown maps for regions.** If your continent map needs a detail-level regional map, a dedicated drilldown-maps guide is queued. Quick version: create both maps, set `parent_map: <continent-map-id>` on the regional map's frontmatter, and the parent map gains a clickable marker leading to the regional map.
...
- [I want to create a fictional universe](/guides/worldbuilding/create-fictional-universe/) — recommended prerequisite for universe-scoped maps
