...
In the body of the saved note, write the analysis: which axis tipped the decision, why the negative evidence supports the conclusion, what would change your mind. See [the proof-summary guide](/guides/research/write-a-proof-summary/) for the full structure.
...
- **Conflict turns out to be two different people.** This happens. Resolve by splitting the person record (or running [duplicate detection in reverse](/guides/research/find-and-merge-duplicates/) — the dismissals are also persistent). The proof summary's conclusion becomes "Sources A and B are evidence about distinct individuals, not the same person."
...
- [I want to write a proof summary for a contested fact](/guides/research/write-a-proof-summary/) — the resolution mechanism in detail
- [I want to track research progress on a long-term project](/guides/research/track-research-progress/) — where the negative-findings log lives
- [I want to set up per-fact source citations](/guides/research/set-up-source-tracking/) — the `sourced_*` arrays that feed into conflict detection
